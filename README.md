...
classDiagram
    ...
    namespace Game {
        ...
        class Worker {
            ...
            +tick()
            +buildStateSnapshot()
            +handleGameCommand(command, requestId, payload)
        }
    }
    ...
    namespace EntityPkg {
        ...
        class Unit {
            ...
            +updateTarget(enemies, attackRange, retainRange)
            +addBuff(newBuff)
            +removeBuff(buffName)
            +updateRegeneration()
            +updateBuffs()
    ...
    namespace HeroUnits {
        class Hero {
            ...
            +startCast(duration, onComplete, onTick)
            +interruptCast()
            +isCasting()
            +applyPassiveSkills()
            +updatePassiveSkills()
    ...
    namespace EnemyUnits {
        ...
        class Lich {
            ...
            +delayThenCastQ()
            +castBlink(distance, randomDirection)
            +updatePhase()
            +updateBehavior()
            +update()
        }
    ...
    namespace Skills {
        class Skill {
            ...
            +sufficientMP(MP)
            +casted()
            +getAttackDamage(baseDamage, caster, options)
        ...
        class BossSkill {
            +number castDuration
            +number backswingDuration
            +number castRemaining
            +number backswingRemaining
            +object castContext
            +string phase
            +startCast(context)
            +updateCasting()
    ...
    WorldPkg.World <|-- WorldPkg.Forest
    EntityPkg.Entity <|-- Missile
    EntityPkg.Entity <|-- Area
    EntityPkg.Entity <|-- Aura
    EntityPkg.Entity <|-- Projectile
    Tower <|-- FlameTowerEntity
    Tower <|-- FrostTowerEntity

    Skill <|-- BossSkill
    BossSkill <|-- IceBolt
    BossSkill <|-- FrostBlast
    BossSkill <|-- Blink

    %% Skill Implementation Inheritance
    Skill <|-- Slash
    Skill <|-- BladeSpin
    Skill <|-- Sacrifice
    Skill <|-- JumpingSlash
    Skill <|-- EarthquakeSlash
    Skill <|-- Sanguivore
    Skill <|-- Stab
    Skill <|-- Puncture
    Skill <|-- Parry
    ...
    Skill <|-- BallLightning
    Skill <|-- StaticExplosion
    Skill <|-- ElectromagneticField

    %% Composition / Aggregation
    GameManager *-- Clock
    GameManager *-- EventEmitter
    GameManager *-- WorldPkg.World
    GameManager *-- Hero
    GameManager *-- Objective
    GameManager o-- "0..*" Enemy
    GameManager o-- "0..1" Boss
    GameManager o-- "0..*" EntityPkg.Entity : skillEntities
    
    WorldPkg.World *-- "0..*" Wave
    Wave *-- "1..*" Lane
    ...
    Area --> Unit : hits
    Aura --> Unit : affects
    Projectile --> Unit : hits
    Guardian --> Missile : creates
    Tower --> Projectile : creates
    Tower --> Missile : creates

    ArrowTower ..> Tower
    RockTower ..> Tower
    FlameTower ..> FlameTowerEntity
    FrostTower ..> FrostTowerEntity
    Demolish ..> Tower
    ViperGuardian ..> Guardian
    ForeSight ..> SeeThroughDecoy
