...
Once a skill has acquired a valid target, it calls the `casted` function, within which you can choose to cast a `SkillEntity`, apply a `Buff`, move the hero’s position, and so on.
```JavaScript
...
Buffs exist on attached units and affect them, such as slowing them down. Most buffs have a duration; after the duration ends, the buff disappears, and its effects also disappear. When a unit is affected by multiple buffs with the same name, only the last buff applied will retain its effect.
...
```
Buff status is updated every frame, which may alter the duration or the target unit. To prevent multiple units from sharing a single Buff pointer, multiple Buffs are created here to affect each unit individually.
```JavaScript
