...
class Main {
    ...
    +drawWorld(sketch, state)
    +drawObjective(sketch, objective)
    +drawTargetingOverlay(sketch, state)
}
...
class Render {
    ...
    +withWorldTransform(callback)
    +renderBaseRing(objective)
...
Main --> MenuScene
Main --> IntroScene
Main --> InstructionScene

MenuScene *-- Button
IntroScene *-- Button
InstructionScene *-- Button
...
BallLightning --|> Skill
StaticExplosion --|> Skill
ElectromagneticField --|> Skill
...
Area --|> SkillEntity
Aura --|> SkillEntity
Projectile --|> SkillEntity
Guardian --|> SkillEntity

Skill ..> SkillEntity
GameManager o-- SkillEntity
```
